...
class Assembler:
   ...
   def assembleLine(self, line):
      ...
      if instruction in ['add', 'addu', 'and', 'or', 'nor', 'slt', 'sltu', 'sub', 'subu']:
         ...
         src = arg2
         imm = arg3
         imm = self.getOffsetFromArg(imm, self.line_number)
   ...
   def exportData(self, filename):
      ...
      with open(filename, 'w') as outputFile:
         ...
         output = "    [" + '{0:03X}'.format(i) + ".." + '{0:03X}'.format(self.DEPTH - 1) + "]"
         output += "  :   ";
...
def main():
   for i in range(1, len(sys.argv)):
      assembler = Assembler()
      assembler.setFileInput(sys.argv[i])
      try:
